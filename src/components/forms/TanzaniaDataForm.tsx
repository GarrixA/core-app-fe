...
const TanzaniaDataForm = () => {
  const location = useLocation();
  const crops = location.state?.crops ?? [location.state?.crop].filter(Boolean);
  ...
  const {
    ...
    defaultValues: {
      crops: crops.map((crop: CropRow) => ({
        crop_country: crop.crop_country || "Tanzania",
...
export default TanzaniaDataForm;
