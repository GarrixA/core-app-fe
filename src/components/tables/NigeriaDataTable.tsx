...
const NigeriaDataTable = () => {
  const { data: crops = [], isLoading, isError } = useGetCropsQuery();

  return (
    <div className="overflow-x-auto mt-8">
      <h1 className="text-2xl font-bold my-2">Nigeria data table</h1>
      {isLoading ? (
        <div>Loading...</div>
      ) : isError ? (
        ...
        <table className="min-w-full bg-white border border-gray-200 rounded-lg text-xs">
          ...
          <tbody>
            {crops?.data
              ?.filter((crop: any) => crop?.country?.toLowerCase() === "nigeria")
              .slice()
...
export default NigeriaDataTable;
